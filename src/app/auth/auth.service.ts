...
export class AuthService {
    ...
    signup(email: string, password: string){
        return this.http.post<AuthResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBaH-TPp3kSWxH1hbo8uvWwmwILmGBzxDs',{
            ...
            this.handleAuthentiation(email, id, token, +expiresIn)
            //const user = new User(emial,idToken)
        }))
    }
    ...
    private handleAuthentiation(email: string, id: string, token: string, expiresIn: number){
